...
flowchart LR
  ...
  pkg_channel["channel"]
  svc_channel["ctx.channel<br/>Authenticated text-channel seam"]
  pkg_channel_telegram["channel-telegram"]
  pkg_channel_agent["channel-agent"]
  svc_channelTelegram["ctx.channelTelegram<br/>Telegram transport and local pairing control"]
  pkg_ui_settings_channel_telegram["ui-settings-channel-telegram"]
  pkg_spill["spill"]
  ...
  pkg_plugin_marketplace["plugin-marketplace"]
  svc_pluginMarketplace["ctx.pluginMarketplace<br/>Third-party plugin discovery"]
  ...
  pkg_channel --> svc_channel
  pkg_channel_telegram --> svc_channel
  pkg_channel_telegram --> svc_channelTelegram
  ...
  pkg_plugin_marketplace --> svc_pluginMarketplace
  ...
  svc_channel --> pkg_channel_agent
  svc_channelTelegram --> pkg_ui_settings_channel_telegram
  ...
  svc_pluginMarketplace --> pkg_ui_settings_plugin_inventory
...
| `ctx.channel` | `seam` | [`channel`](../packages/interaction/channel) | [`channel-telegram`](../packages/interaction/channel-telegram) | [`channel-agent`](../packages/interaction/channel-agent) | - | 经过身份验证的传输提供方将规范化文本交给一个产品消费方；Agent 消费方负责持久化幂等，并通过消息来源提供方把最终文本发回。 |
| `ctx.channelTelegram` | `core` | [`channel-telegram`](../packages/interaction/channel-telegram) | - | `ui-settings-channel-telegram` | - | 使用出站 Bot API 长轮询、精确的私聊身份和仅限回环地址的两步配对 Remote；它绝不公开 Host API，也不开放公网监听器。 |
...
| `ctx.pluginMarketplace` | `core` | `plugin-marketplace` | - | `ui-settings-plugin-inventory` | - | 缓存有上限的 GitHub topic 发现结果，只在固定 commit 校验被选中的仓库，并在安装保持 fail-closed 时返回风险预览。 |
