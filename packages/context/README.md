...
Product plugins that add model-visible request context without defining a tool. `agent-instructions` is included by the default `dsh-agent-spine-demo` bundle and can be disabled through bundle config; `user-profile` is included by the Desktop Web composition but emits only fields with explicit per-field consent; `time-context`, `tmux-context`, and `session-reference` are opt-in.
