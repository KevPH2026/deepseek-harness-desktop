...
import type {
  ...
  PluginMarketplaceFeaturedPluginsResult,
  PluginMarketplaceFeaturedPluginsSnapshot,
...
export function withoutCuratedDisableSection(source: string): string {
  // The managed disable block sits in its own YAML document, separated
  // from the user-authored list by a '---' line. Strip from the
  // separator (if any) to the end of CURATED_DISABLE_END, then drop
  // the trailing whitespace so the caller sees the user content
  // unchanged.
  const sepIdx = source.lastIndexOf('\n---\n')
  if (sepIdx < 0) {
    // No separator: the block is either absent or written as the
    // first document, in which case leave the source alone (the
    // install path is the only writer that inserts the separator).
    return source
  }
  return source.slice(0, sepIdx).trimEnd() + '\n'
}
...
export class PluginMarketplaceGateway extends TypertRemoteService {
   ...
   */
  /**
   * One locally-bundled / radar-fetched "Featured picks" snapshot for the
   * marketplace top bar. Generated by `pnpm sync:featured` and shipped
   * as `featured-plugins.generated.json`; on a build that did not run
   * the sync the bundled fallback (`featured-plugins.fallback.json`)
   * keeps the UI populated.
   * @returns Snapshot items + source tag + last-refresh timestamp.
   */
  @Remote('featuredPlugins')
  async featuredPlugins(): Promise<PluginMarketplaceFeaturedPluginsResult> {
    // The JSON files ship with the host bundle, not the runtime profile.
    // Resolve from the host package's own location at module-init time.
    const hostRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')
    for (const file of ['featured-plugins.generated.json', 'featured-plugins.fallback.json']) {
      try {
        const text = await readFile(join(hostRoot, 'src', file), 'utf8')
        const snapshot = JSON.parse(text) as PluginMarketplaceFeaturedPluginsSnapshot
        return { ok: true, snapshot }
      } catch {
        // Try the next file or report failure below.
      }
    }
    return {
      ok: false,
      errorCode: 'fetch-failed',
      errorMessage: 'No featured-plugins bundle shipped with this build.',
    }
  }
